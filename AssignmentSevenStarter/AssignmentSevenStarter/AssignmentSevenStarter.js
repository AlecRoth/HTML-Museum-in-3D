...
//    it's a bit of a hack, but works well enough for simple navigation
...
function loadPoints(points,texture) {
    ...
    //TWO TRIANGLe FOR The each wall, for floor, and for pictures. should be tweleve groups of 3;
...
//ART - mona lisa
    ...
    texture.push(vec2(.5,1));
    points.push(vec4(-2.0, 1, 0.1, 1));
    texture.push(vec2(.5,.5));
    points.push(vec4(2, 4,0.1, 1));
...
//MILK
    ...
    points.push(vec4(5.9, 6, 8, 1));
    texture.push(vec2(1,.5));
    points.push(vec4(5.9, 0.5, 8, 1));
    texture.push(vec2(1,0));
    points.push(vec4(5.9, 6, 2, 1));
    texture.push(vec2(.5, .5));
